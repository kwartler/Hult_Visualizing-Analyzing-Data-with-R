...
# Get state data
stateToGet <- 'CA'
iceCream <- readr:: read_csv('https://raw.githubusercontent.com/kwartler/Hult_Visualizing-Analyzing-Data-with-R/main/DD1/B_Mar2/lab/mcBroken_august_12_2022.csv')
...
worstCities <- aggregate(is_broken~ city + date, iceCream, sum)
badCity <- worstCities[order(worstCities$is_broken, decreasing=T)[2],] 

# Subset the original data to the city being investigated
plotDF <- iceCream[iceCream$city %in% badCity$city,] 

ggplot(data = plotDF) +
  geom_point( aes(x= last_checked, y = street, shape = is_broken, color = is_broken))+
  theme(axis.text.x=element_text(size=1.5)) +
  theme_minimal() +
  ggtitle(paste('Sensor Reads by Time & Location at:',plotDF$city[1]))
```
